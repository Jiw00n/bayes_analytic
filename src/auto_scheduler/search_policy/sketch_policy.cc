...
Array<State> SketchPolicyNode::SampleInitPopulation(const Array<State>& sketches) {
  ...
  while (static_cast<int>(out_states.size()) < sample_init_min_pop_) {
    ...
    // 실험용
    // -exec call InferboundAndPrint_(this, &tmp_s)
    // ======================================================================
    // int index = 0;
    
    // std::cout << "Sketch : \n";
    // DBG_PrintState(sketches[0], false, true);

    // while(index < population){
    //   State tmp_s = sketches[0];
    //   // init fill tile size
    //   auto validation0 = init_rules[0]->Apply(this, &tmp_s, &rand_gens[index]);
    //   if (PopulationGenerationRule::ResultKind::kInvalid == validation0) continue;
      
    //   // init thread bind
    //   auto validation1 = init_rules[1]->Apply(this, &tmp_s, &rand_gens[index]);
    //   if (PopulationGenerationRule::ResultKind::kInvalid == validation1) continue;

    //   // init unroll
    //   auto validation2 = init_rules[2]->Apply(this, &tmp_s, &rand_gens[index]);
    //   if (PopulationGenerationRule::ResultKind::kInvalid == validation2) continue;
...
Array<State> SketchPolicyNode::SampleInitPop_Rule_tgt(const Array<State>& sketches, int target_rule) {
  ...
  // 1, 12, 13, 123
  std::vector<int> target_init_rules;
  target_init_rules.push_back(0); // fill tile size
  if (target_rule == 12 || target_rule == 123){
    target_init_rules.push_back(1);
  }
  if (target_rule == 13 || target_rule == 123){
    target_init_rules.push_back(2);
  }
  ...
  while (static_cast<int>(out_states.size()) < 1) {
    ...
    support::parallel_for(0, population, [this, &temp_states, &sketches, &rand_gens, &target_init_rules](int index) {
      // Randomly choose a sketch
      State tmp_s = sketches[0];
      // Apply random annotation rules one by one
      bool valid = true;
      for (const int rule_idx : target_init_rules) {
        const auto&rule = init_rules[rule_idx];
        if (rule->Apply(this, &tmp_s, &rand_gens[index]) ==
            PopulationGenerationRule::ResultKind::kInvalid) {
          valid = false;
          break;
    ...
    //   // init fill tile size
    //   if (target_rule == 1 || target_rule == 12 || target_rule == 13 || target_rule == 123){
    //     while (!valid) {
    //       auto validation = init_rules[0]->Apply(this, &tmp_s, &rand_gens[index]);
    //       valid = PopulationGenerationRule::ResultKind::kValid == validation ? true : false;
    //     }
    //     valid = false;
    //   }
      
      
    //   if (target_rule == 2 || target_rule == 12 || target_rule == 23 || target_rule == 123){
    //   // init thread bind
    //     while (!valid) {
    //       auto validation = init_rules[1]->Apply(this, &tmp_s, &rand_gens[index]);
    //       valid = PopulationGenerationRule::ResultKind::kValid == validation ? true : false;
    //     }
    //     valid = false;
    //   }
    ...
    //   // init unroll
    //   if (target_rule == 3 || target_rule == 13 || target_rule == 23 || target_rule == 123){
    //     while (!valid) {
    //       auto validation = init_rules[2]->Apply(this, &tmp_s, &rand_gens[index]);
    //       valid = PopulationGenerationRule::ResultKind::kValid == validation ? true : false;
    //     }
    //   }
...
// -exec call InferboundAndPrint_(this, &tmp_s)
void InferboundAndPrint_(const SketchPolicyNode* policy, State* state) {
  State new_state = policy->search_task->compute_dag.InferBound(*state);
  std::cout << "After InferBound: \n";
  DBG_PrintState(*state, false, true);
}
...
.set_body_typed([](SketchPolicy policy, int target_rule) {
  ...
  Array<State> init_population = policy->SampleInitPop_Rule_tgt(sketches, target_rule);
  return init_population;
});

TVM_REGISTER_GLOBAL("auto_scheduler.CustomPrintState_").set_body_typed([](const State& s, bool delete_trivial_loop, bool show_dag) {
  std::ostringstream oss;
  PrintState(&oss, s, delete_trivial_loop, show_dag);
  return oss.str();
});
