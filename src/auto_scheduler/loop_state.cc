...
void PrintStage(std::ostream* os, int stage_id, const State& state, size_t base_indent,
                bool delete_trivial_loop, bool show_dag) {
    ...
    if (state.defined()) {
      ...
      if (pair != state->attach_map->iter_to_attached_stages.end()) {
        ...
        for (const auto& attach_stage_id : pair->second) {
          PrintStage(os, attach_stage_id, state, base_indent + indent, delete_trivial_loop, show_dag);
        }
      }
    }
  }
...
void PrintState(std::ostream* os, const State& state, bool delete_trivial_loop, bool show_dag) {
  // Gather placeholders
  ...
  for (size_t i = 0; i < state->stages.size(); ++i) {
    ...
    if (stage->op_type == StageKind::kPlaceholder) {
      ...
      if (stage->compute_at == ComputeAtKind::kRoot) {
        PrintStage(os, i, state, 0, delete_trivial_loop, show_dag);
      }
    } else {
...
TVM_STATIC_IR_FUNCTOR(ReprPrinter, vtable)
    .set_dispatch<StateNode>([](const ObjectRef& ref, ReprPrinter* p) {
      PrintState(&p->stream, tvm::Downcast<State>(ref), false);
    });
