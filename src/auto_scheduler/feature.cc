 ...
 public:
  ...
  void VisitStmt_(const AttrStmtNode* node) final {
    if (node->attr_key == tir::attr::thread_extent || node->attr_key == tir::attr::virtual_thread) {
      ...
      // for문 같은 statement가 같은 레벨에 있을 수 있기 때문에 pop해주고, extent 나눠줌
  ...
  void VisitStmt_(const ForNode* node) final {
    // ana_ : 수식 해석기 (arith::analyzer)
      // loop_var이 min-extent 안에서 돈다는 것을 기록
      // 이후 루프 내부의 계산을 단순화하거나, buffer access의 feature를 계산할 때 사용
    ...
    // loop 안에서 정의된 variable 추적
    variable_definition_stack_.push_back({});

    // StmtExprVisitor의 VisitStmt_를 호출해서 body를 방문
    // 그렇게 자식 노드들을 방문
      // for문은 min(expr), extent(expr), body(stmt)를 방문
    StmtExprVisitor::VisitStmt_(node);
    variable_definition_stack_.pop_back();
    for_loop_stack_.pop_back();
    outer_loop_prod_ /= loop_extent;
