...
void PrintStage(std::ostream* os, int stage_id, const State& state,
                size_t base_indent = 0, bool delete_trivial_loop = true, bool show_dag = false);
void PrintState(std::ostream* os, const State& state, bool delete_trivial_loop = true, bool show_dag = false);
...
class Stage : public ObjectRef {
 public:
   ...
   */
  explicit Stage(te::Operation op);     // 암시적 변환 금지
  
  // explicit이 없으면 암시적 변환이 허용된다. (Stage 객체가 들어갈 자리에 te::Operation가 들어갈 수 있음)
  ...
  // ex.2
    // void foo(Stage s){ ... }
    // foo(te::Operation op)  가능

  // Stage 생성자가 te::Operation 객체를 받아서 Stage 객체로 변환해주기 때문
  // 생성자의 매개변수가 하나인 경우에만 가능
