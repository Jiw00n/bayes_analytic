...
from .search_policy import SearchPolicy, SketchPolicy, PreloadMeasuredStates, CustomPrintState
...
def make_search_policies(
    ...
    if isinstance(search_policy, str):
        ...
        if model_type == "xgb":
            # warmup_num = len(tasks) * num_measures_per_round if len(tasks) > 1 else 10000
            cost_model = XGBModel(
...
class TaskScheduler:
    ...
    def __init__(
        ...
        backward_window_size: int = 3,
        callbacks=None,
        tsv_log_path="total_latency.tsv",
    ):
        self.tasks = tasks
        ...
        self.callbacks = (
            ...
            else [PrintTableInfo(), LogEstimatedLatency(tsv_log_path)]
        )
    ...
    def tune(
        ...
        def log_state(state, file_name, delete_triv=False, show_dag=True):
            state_str = CustomPrintState(state, delete_triv, show_dag)
        ...
        # log_state(init_pop[0], file_name)
        # for i in range(30):
        #     evos = self.search_policies[0].evolutionary_search(init_pop, 100)
        #     for evo in evos:
        ...
        #         #     break
        #         for i, policy in enumerate(self.search_policies):

        #             init_pop = policy.SampleInitPop_Rule_tgt(rule)[0]
        #             init_pop_str = CustomPrintState(init_pop, delete_triv, False)
        #             # evos = policy.evolutionary_search(init_pop, 100)
        #             print(policy.search_task.desc)
        #             # for idx, pop in enumerate(init_pop):
        #             with open(file_name, "a") as f:
        #                 # if idx == 0:
        #                 f.write(f"{policy.search_task.desc}\n")
        #                 f.write(f"{policy.search_task.workload_key}\n")
        #                 f.write(init_pop_str + "\n\n\n")
                    
        #             # for idx, evo in enumerate(evos):
        #             #     with open(f"states/res_18/evo_{i}.txt", "a") as f:
        #             #         if idx == 0:
        #             #             f.write(f"{self.tasks[i].workload_key}\n\n")
        #             #         f.write(str(evo) + "\n")    
