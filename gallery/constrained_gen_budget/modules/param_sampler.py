...
class ParamSampler:
    ...
    @staticmethod
    def _copy_budget_remaining(budget_remaining):
        return {name: int(value) for name, value in budget_remaining.items()}
    ...
    def _initialize_unique_search_base_state(self, var_order):
        ...
        domains.update(g._build_budget_domains())
        group_remaining = {}
    ...
    def _compute_reachable_products(self, candidate_lists, limit):
        reachable = {1}
        limit = int(limit)
    ...
    def _get_budget_candidates(self, name, domains, group_remaining, result):
        ...
        for factor_name in spec['factor_names']:
            candidates = self._get_split_candidates_base(
                factor_name,
                domains,
                group_remaining,
            )
        ...
        reachable = self._compute_reachable_products(candidate_lists, spec['limit'])
    ...
    def _split_candidates_with_budget(
        self,
        name,
        candidates,
        domains,
        ...
        target = budget_remaining.get(budget_name)
        if target is None:
            return candidates
        ...
        for candidate in candidates:
            candidate = int(candidate)
            if candidate <= 0 or target % candidate != 0:
                continue
            rest_target = target // candidate
            remaining_lists = []
            for factor_name in spec['factor_names']:
                if factor_name == name or factor_name in result:
                    continue
                factor_candidates = self._get_split_candidates_base(
                    factor_name,
                    domains,
                    group_remaining,
                )
                if not factor_candidates:
                    ...
                    break
                remaining_lists.append(factor_candidates)
            if remaining_lists is None:
                continue
            if not remaining_lists:
                if rest_target == 1:
                    filtered.append(candidate)
                continue
            reachable = self._compute_reachable_products(remaining_lists, rest_target)
            if rest_target in reachable:
                filtered.append(candidate)
        return filtered
    ...
    def _apply_search_assignment(self, name, chosen, result, domains, group_remaining, budget_remaining):
        ...
        if g._is_budget_name(name):
            ...
            fixed_product = 1
            for factor_name in spec['factor_names']:
                if factor_name in result and factor_name != name:
                    fixed_product *= int(result[factor_name])
            if fixed_product <= 0 or chosen % fixed_product != 0:
                budget_remaining[name] = 0
            else:
                budget_remaining[name] = chosen // fixed_product
            domains[name] = chosen
            return
        ...
        spec = g._budget_spec_by_factor.get(name)
        if spec is not None and spec['name'] in budget_remaining and chosen > 0:
            budget_remaining[spec['name']] = int(budget_remaining[spec['name']]) // chosen
    ...
    def _random_unique_rollout(
        ...
        budget_remaining = self._copy_budget_remaining(state['base_budget_remaining'])
        path_records = []
