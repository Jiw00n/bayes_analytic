...
def state_cost_to_records(states, costs, cost_type="neg_log"):
    ...
    for i, schedule in enumerate(records["schedules"]):
        ...
        all_features = extents+unrolls
        records["all"].append(np.array(all_features, dtype=np.float32))
...
# state 받아서 extent, unroll 정보가 포함된 records 딕셔너리 반환
def state_to_records(states, ret_type="records"):

    if isinstance(states, set):
        if len(states) == 0:
            return {"schedules": [], "extents": [], "unroll" : [], "all": []}
        if isinstance(next(iter(states)), str):
            state_strs = states
        ...
        state_strs = list(map(lambda x: str(x).strip(), list(states)))
...
# 프로그램 풀 생성하고 extent 포함된 records 딕셔너리 반환
