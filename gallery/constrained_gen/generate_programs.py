...
def process_task(task_index, task, records_per_task=1, emit_logs=True):
    ...
    output_path = get_to_measure_gen_filename(task)
...
def _run_task_subprocess(task_index, records_per_task=1):
    """해당 task만 처리하는 워커 서브프로세를 실행해 완료 결과를 반환한다."""
    cmd = [
        sys.executable,
...
def _extract_subprocess_result(stdout):
    """서브프로세 stdout에서 JSON 결과(task_result)와 그 외 출력 줄을 분리해 반환한다."""
    relay_lines = []
    result = None
...
def _collect_subprocess_result(task_index, task, completed):
    ...
    _emit_failure(task_index, task, "worker_process", None, error)
    return {
        "ok": False,
...
def run_selected_tasks(
    ...
    if workers == 1 or len(selected_tasks) <= 1:
        results = []
        progress_ctx = (
            tqdm(total=total_tasks, desc="tasks", unit="task")
            if not suppress_progress_output
            else nullcontext()
        )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        for task_index, task in selected_tasks:
            future = executor.submit(_run_task_subprocess, task_index, records_per_task)
        ...
        with progress_ctx as pbar:
            for future in concurrent.futures.as_completed(future_map):
                ...
                try:
                    ...
                    results.append(_collect_subprocess_result(task_index, task, completed))
                if pbar is not None:
                    pbar.update(1)
    return results
