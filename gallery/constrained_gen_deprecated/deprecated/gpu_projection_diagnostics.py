"""Diagnostic-only helpers for projected-vs-exact GPU constraint triage."""
# 제거 필요
...
def _kind_snapshot(gen, params, kind, exact_tree, projected_tree, limit):
    """한 제약 종류에 대해 projected/exact 값과 limit 대비 통과 여부를 딕셔너리로 반환한다."""
...
def _gpu_error_kind(message):
    """GPU 검증 에러 메시지에서 제약 종류(예: max_threads, shared_memory)를 추출한다."""
    if (
...
def _simplify_primfunc(func):
    """PrimFunc에 Simplify 패스를 적용한 뒤 반환한다."""
...
def _substitute_params_in_primfunc(func, params):
    """PrimFunc 내 변수를 params 값으로 치환한 새 PrimFunc를 반환한다."""
    subst = {}
...
def _finite_interval_upper(expr, domains):
    """expr의 구간 상한을 정수로 반환한다 (무한대면 None)."""
...
def _collect_exact_lowering_differential(gen, params, max_cases=8):
    """exact 케이스별 하이브리드 검증 차이 정보를 수집해 진단용 딕셔너리를 반환한다."""
    if not gen._has_concrete_final_context():
        return None
    ...
    concrete_errors = []
    concrete_result = gen._get_concrete_final_result(params)
...
def _classify_exact_false_reject(kind, exact_lowering_differential):
    """exact false reject의 근본 원인 문자열을 분류해 반환한다."""
...
def collect_gpu_projection_diagnostics(gen, params):
    """projected/exact 제약 스냅샷과 위반·근본원인을 담은 진단 딕셔너리를 반환한다."""
    gen._ensure_exact_gpu_constraints()
    ...
    return {
        "projected_constraints": gen._get_constraints_str(include_vars=True),
        "raw_exact_constraints": gen._get_raw_exact_constraints_str(include_vars=True),
        "snapshots": snapshots,
...
def collect_false_reject_diagnostics(gen, params, violations):
    """false reject 원인 분석용 스냅샷·근본원인·구체 결과를 담은 딕셔너리를 반환한다."""
    gen._ensure_exact_gpu_constraints()
    gen._ensure_projected_gpu_constraints()
    snapshots = []
    snapshot_by_kind = {}
