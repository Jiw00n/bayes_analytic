...
def _load_measurement_lookup(path: Path) -> dict:
    ...
    Missing files return an empty dict."""
    cache: dict = {}
    if not path.exists():
        return cache
...
def _save_measurement_lookup(path: Path, cache: dict) -> int:
    """Persist all usable measurements in ``cache`` to ``path`` (JSONL, one
    ``{"sym_map": ..., "cost": ...}`` per line). Returns the number of entries
    written. Non-usable or non-finite entries are skipped."""
    ...
    with tmp.open("w", encoding="utf-8") as f:
        for sym_key, entry in cache.items():
            ...
            f.write(
                json.dumps(
                    {"sym_map": sym_map, "cost": cost_f}, ensure_ascii=False
                )
                + "\n"
            )
            written += 1
        ...
        if walk_due or recon_predict_due:
            ...
            if added:
                _save_measurement_lookup(
                    measurement_lookup_path, persistent_measurement_cache
                )
                print(
