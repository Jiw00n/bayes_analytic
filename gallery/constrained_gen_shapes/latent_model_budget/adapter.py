...
def _parse_shapes_from_workload_key(workload_key: Optional[str]) -> Optional[Tuple[Tuple[int, ...], ...]]:
    """workload_key is a json-encoded [hash, shape_list_1, shape_list_2, ...]."""
    if workload_key is None:
        return None
    try:
        parts = json.loads(workload_key)
    ...
    for entry in parts[1:]:
        if not isinstance(entry, (list, tuple)):
            return None
        shapes.append(tuple(int(v) for v in entry))
    return tuple(shapes)
