...
class ParamTokenizer:
    ...
    UNK = "<UNK>"
    PARAM_START_VAR = "__param_start__"
    ...
    def __init__(self, state: ParamTokenizerState):
        ...
        self.param_start_id = self.token_to_id[self.PARAM_START]
        self.unk_id = self.token_to_id[self.UNK]
        self.var_pad_id = self.var_to_id["<VAR_PAD>"]
        self.param_start_var_id = self.var_to_id[self.PARAM_START_VAR]
    ...
    def token_to_value(var_name: str, token: str) -> int | None:
        if token in {ParamTokenizer.PAD, ParamTokenizer.PARAM_START, ParamTokenizer.UNK}:
            return None
        if var_name.startswith("ur_"):
    ...
    def build(
        ...
        tokens: List[str] = [cls.PAD, cls.PARAM_START, cls.UNK]
        ...
        vars_: List[str] = ["<VAR_PAD>", cls.PARAM_START_VAR]
        var_seen = set(vars_)
